...
if (!gotSingleInstanceLock) {
  log.info(`Another ${APP_NAME} instance is already running. Exiting duplicate instance.`);
  app.quit();
} else {
  app.on('second-instance', () => {
    ...
    tray?.displayBalloon?.({ title: APP_NAME, content: 'O Bridge já está em execução na bandeja do Windows.' });
  });
}
...
function openKiosk({ force = false } = {}) {
  if (!force && kioskOpenedByThisInstance) return;
...
function rebuildMenu() {
  ...
  const menu = Menu.buildFromTemplate([
    ...
    { label: 'Abrir status local', click: () => shell.openExternal(`http://localhost:${state.port || 8765}/status`) },
...
async function restartBridge() {
  try {
    ...
    dialog.showErrorBox(APP_NAME, `Falha ao iniciar bridge: ${error.message || error}`);
  } finally { rebuildMenu(); }
}
...
function setupAutoUpdaterEvents() {
  autoUpdater.on('checking-for-update', () => { updateCheckInProgress = true; rebuildMenu(); });
  autoUpdater.on('update-not-available', () => {
    if (manualUpdateCheck) dialog.showMessageBox({ type: 'info', title: APP_NAME, message: 'Nenhuma atualização disponível', detail: `Versão atual: ${app.getVersion()}` });
  });
  autoUpdater.on('error', (error) => {
    if (manualUpdateCheck) dialog.showErrorBox(APP_NAME, `Falha ao checar atualização.\n\nDetalhe: ${error.message || error}`);
  });
  autoUpdater.on('update-downloaded', (info) => {
    dialog.showMessageBox({ type: 'info', buttons: ['Instalar agora', 'Instalar ao sair'], title: APP_NAME, message: 'Atualização pronta para instalar', detail: `A versão ${info.version || 'mais recente'} foi baixada.` }).then(({ response }) => {
